...
transformed parameters{
  ...
  for (i in 1:N){
    ...
    wa[i] = (aw1[i]+aw2[i]+aw3[i])/Atotw;
    ca[i] = (ac1[i]+ac2[i]+ac3[i])/Atotc;
    pa[i] = wa[i]*ca[i];
...
model{
  ...
  target += beta_proportion_lpdf(w | wa+0.01, kappa);
  target += beta_proportion_lpdf(c | ca+0.01, kappa);
  target += beta_proportion_lpdf(p | pa+0.01, kappa);
...
generated quantities{
 vector[N] log_lik;
    ...
    for(i in 1:N){
      log_lik[i] = beta_proportion_lpdf(w[i] | wa[i]+0.001, kappa)+beta_proportion_lpdf(c[i] | ca[i]+0.001, kappa)+beta_proportion_lpdf(p[i] | pa[i]+0.01, kappa);
 }
