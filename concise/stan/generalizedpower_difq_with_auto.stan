functions {
  ...
  vector get_points(real Tw1, real Tw2, real Tc1, real Tc2,real q1, real q2, real n, real m){
    real  x1;
    real  x2;
    real  y1;
    real  y2;
  ...
  vector get_end_areas(real Tw1, real Tw2, real Tc1, real Tc2,real q1, real q2, real n, real m, vector points){
    real a1_w;
    real a2_w;
    real a1_c;
    real a2_c;
...
parameters {
  real<lower=20, upper =100> q1;
  real<lower=20, upper =100> q2;
...
transformed parameters{
  ...
  for (i in 1:N){
    // [aw1,aw2,ac1,ac2]
    aw1[i] = get_end_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[1];
    aw2[i] = get_end_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[2];
    ac1[i] = get_end_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[3];
    ac2[i] = get_end_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[4];
    // [aw3, ac3]
    aw3[i] = get_middle_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[1];
    ac3[i] = get_middle_areas(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m,get_points(Tw1[i],Tw2[i],Tc1[i],Tc2[i],q1,q2,n,m))[2];
    ...
    if(i == 1){
      ...
      if(wa1[i]+bw*wa[i-1] < 0){uw[i] = 0;}else{uw[i] = wa1[i]+bw*wa[i-1];}
      if(ca1[i]+bc*ca[i-1] < 0){uc[i] = 0;}else{uc[i] = ca1[i]+bc*ca[i-1];}
      if(pa1[i]+bp*pa[i-1] < 0){up[i] = 0;}else{up[i] = pa1[i]+bp*pa[i-1];}
...
model{
  ...
  target += lognormal_lpdf(q2|3,0.5);
  
  target += lognormal_lpdf(n|0,0.8);
  target += lognormal_lpdf(m|0,0.8);
  target += lognormal_lpdf(kappa|3.5,0.5);
  
  target += normal_lpdf(bw|0,0.5);
  target += normal_lpdf(bc|0,0.5);
  target += normal_lpdf(bp|0,0.5);
