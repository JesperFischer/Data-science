...
transformed parameters{
  ...
  for (i in 1:N){
    ...
    pa1[i] = wa1[i]*ca1[i];
      ...
      if(i == 1){
        //percept is a weighting of what have before (wa[i]) and what just came wa1[i] first trial wa[i] =0
        ...
        if(wa[i-1]+(1/(1+bw))*(wa1[i]-wa[i-1]) < 0){uw[i] = 0;}else{uw[i] = wa[i-1]+(1/(1+bw))*(wa1[i]-wa[i-1]);}
        if(ca[i-1]+(1/(1+bc))*(ca1[i]-ca[i-1]) < 0){uc[i] = 0;}else{uc[i] = ca[i-1]+(1/(1+bc))*(ca1[i]-ca[i-1]);}
        if(pa[i-1]+(1/(1+bp))*(pa1[i]-pa[i-1]) < 0){up[i] = 0;}else{up[i] = pa[i-1]+(1/(1+bp))*(pa1[i]-pa[i-1]);}
...
generated quantities{
 ...
 real prior_kappa;
  ...
  prior_q = lognormal_rng(3,0.5);
  prior_n = lognormal_rng(0,0.8);
  prior_m = lognormal_rng(0,0.8);
  prior_kappa = lognormal_rng(3.5,0.5);
  prior_b = lognormal_rng(0,0.5);
