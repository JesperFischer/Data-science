  ...
  for (i in 1:N){
    ...
    up[i] = wa1[i]*ca1[i];
      ...
      if(i == 1){
        ...
        uw[i] = wa[i]+(1/(1+b))*(wa1[i]-wa[i]);
        uc[i] = ca[i]+(1/(1+b))*(ca1[i]-ca[i]);
        ...
        if(wa[i-1]+(1/(1+b))*(wa1[i]-wa[i-1]) < 0){uw[i] = 0;}else{uw[i] = wa[i-1]+(1/(1+b))*(wa1[i]-wa[i-1]);}
        if(ca[i-1]+(1/(1+b))*(ca1[i]-ca[i-1]) < 0){uc[i] = 0;}else{uc[i] = ca[i-1]+(1/(1+b))*(ca1[i]-ca[i-1]);}
