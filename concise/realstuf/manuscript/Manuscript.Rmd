...
pacman::p_load(brms, tidyverse, here, cmdstanr, loo, extraDistr, tidybayes, furrr,bayesplot, patchwork, progressr, R.utils)
...
my_pkg_fn <- function(x) {
  ...
  future_map(x, ~{
    p()
    Sys.sleep(.2)
    do_parameterrecover5(.x)
  })
}

with_progress({
  result <- my_pkg_fn(data_list)
})
...
model_fitter(data_list[[1]])

modelstack = map_dfr(results, 3)


which.max(modelstack[7,])


modelcom = f[[2]]

f = f[[1]]
highest_mean_row <- f[which.min(f$means), ]

f %>% ggplot(aes(x = names))+theme_classic()+theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  geom_pointrange(aes(x = names, y = means, ymin = means-sds, ymax = means+sds))+
    geom_pointrange(data = highest_mean_row, aes(x = names, y = means, ymin = means - sds, ymax = means + sds), color = "red") +
