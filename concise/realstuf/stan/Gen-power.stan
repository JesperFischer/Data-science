...
transformed parameters{
  ...
  for (i in 1:N){
    ...
    if(i == 1){
      ...
      if(wa[i]+(1/(1+bw))*(wa1[i]-wa[i]) <= 0){uw[i] = 0.01;}
      else if(wa[i]+(1/(1+bw))*(wa1[i]-wa[i]) >= 1){uw[i] = 0.99;}
      else{uw[i] = wa[i]+(1/(1+bw))*(wa1[i]-wa[i]);}
      
      
      
      if(ca[i]+(1/(1+bc))*(ca1[i]-ca[i]) <=0){uc[i] = 0.01;}
      else if(ca[i]+(1/(1+bc))*(ca1[i]-ca[i]) >=1){uc[i] = 0.99;}
      else{uc[i] = ca[i]+(1/(1+bc))*(ca1[i]-ca[i]);}
      
      
  
      if(pa[i]+(1/(1+bp))*(pa1[i]-pa[i]) <= 0){up[i] = 0.01;}
      else if(pa[i]+(1/(1+bp))*(pa1[i]-pa[i]) >= 1){up[i] = 0.99;}
      else{up[i] = pa[i]+(1/(1+bp))*(pa1[i]-pa[i]);}
...
model{
  ...
  target += lognormal_lpdf(n|0,0.5);
  target += lognormal_lpdf(m|0,0.5);
  target += lognormal_lpdf(kappa|3.5,0.5);
  
  target += lognormal_lpdf(bw|0,0.5);
  target += lognormal_lpdf(bc|0,0.5);
  target += lognormal_lpdf(bp|0,0.5);
  
  target += lognormal_lpdf(kw|-0.8,0.6);
  target += lognormal_lpdf(kc|-0.8,0.6);
...
generated quantities{
 ...
 real prior_q2 = lognormal_rng(3,0.5);
 
 real prior_n = lognormal_rng(0,0.5);
 real prior_m = lognormal_rng(0,0.5);
 real prior_kappa = lognormal_rng(3.5,0.5);
  
 real prior_bw = lognormal_rng(0,0.5);
 real prior_bc = lognormal_rng(0,0.5);
 real prior_bp = lognormal_rng(0,0.5);
 
 real prior_kw = lognormal_rng(1,1);
 real prior_kc = lognormal_rng(1,1);
